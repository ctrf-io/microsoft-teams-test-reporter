...
export const formatResultsMessage = (
	...
	if (environment) {
		...
		if (buildName && buildNumber) {
			buildInfo = buildUrl
				? `[${buildName} #${buildNumber}](${buildUrl})`
	...
	const jiraIssueUrl = (ctrf.results.extra as Record<string, unknown>)
		?.jiraIssueUrl as string | undefined;
	const jiraIssueKey = (ctrf.results.extra as Record<string, unknown>)
		?.jiraIssue as string | undefined;
	...
	const facts: any[] = [
		...
		{
			name: "Duration",
			value: `*Duration:* ${duration(summary.start, summary.stop)}`,
		},
...
export const formatFlakyTestsMessage = (ctrf: CtrfReport): object | null => {
	...
	const jiraFlakyIssueUrl = (ctrf.results.extra as Record<string, unknown>)
		?.jiraFlakyIssueUrl as string | undefined;
	const jiraFlakyIssueKey = (ctrf.results.extra as Record<string, unknown>)
		?.jiraFlakyIssue as string | undefined;
	...
	if (jiraFlakyIssueUrl) {
		flakyFacts.push({
			name: "Jira Issue",
			value: jiraFlakyIssueKey
				? `[${jiraFlakyIssueKey}](${jiraFlakyIssueUrl})`
				: `[View Jira Issue](${jiraFlakyIssueUrl})`,
		});
	}
...
export const formatAiSummaryForTest = (
	test: CtrfTest,
	...
	const sections: any[] = [
		{
			...
			facts: [
				...
				{
					name: "&#x2728; AI Summary",
					value: test.ai || "No AI summary provided.",
				},
				{ name: "Build", value: buildInfo },
			],
			markdown: true,
...
export const formatFailedTestsMessage = (ctrf: CtrfReport): string => {
	const failedTests = ctrf.results.tests.filter(
		(test) => test.status === "failed",
	);
	if (failedTests.length === 0) return "No failed tests.";

	const message = failedTests
